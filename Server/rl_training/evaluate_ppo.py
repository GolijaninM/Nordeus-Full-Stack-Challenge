...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate a trained PPO checkpoint with matchup breakdowns.")
    parser.add_argument("--model", default="models/PPO/fair_init/ppo_fair_random_100k/ppo_model.zip")
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Optional text report path. Defaults to <model_dir>/ppo_matchup_breakdown.txt.",
    )
    parser.add_argument(
        "--opponent-models",
        nargs="*",
        default=[],
        help="PPO checkpoint paths to evaluate against in addition to RandomBotPolicy.",
    )
    parser.add_argument(
        "--random-opponents",
        type=int,
...
def main():
    ...
    output_path = Path(args.output) if args.output else model_path.parent / "ppo_matchup_breakdown.txt"
    ...
    for index, (opponent_path, opponent_model) in enumerate(loaded_opponent_models, start=1):
        reports.append((
            f"PPO vs opponent model {index}: {_format_model_label(opponent_path)}",
            evaluate_policy(
                model=model,
                episodes=args.episodes,
    ...
    if args.symmetric_eval:
        reports.append((
            "PPO symmetric tournament vs mixed opponent pool",
